...
const checkJWT = async (req: CustomRequest, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const { rows } = await db.query('SELECT * FROM users WHERE nickname = $1 AND accessToken = $2', [nickname, token]);
